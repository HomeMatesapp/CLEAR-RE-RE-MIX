// Registered Nurse flavor for the shared modular payload builder.
//
// NOTE: The nurse adapter builds the `qualification_verification_required`
// payload directly (not via the shared builder) because that status must
// mix `investigate_after_check` (the primary verification card) and
// `may_open_later` (UK alternative routes) card kinds — a mix the shared
// builder does not produce. `investigateAfterCheckFit` here is used by the
// adapter for the primary verification card.

import type { ModularPayloadFlavor } from "./modular-payload";
import type { RegisteredNurseRouteId } from "./registered-nurse";

export const registeredNurseFlavor: ModularPayloadFlavor<RegisteredNurseRouteId> = {
  questionLabels: {
  ...
  cautionCard: {
    ...
    fit:
      "Some providers market courses using the word \"nursing\" that are not on the NMC approved-programmes list.",
    constraint:
      "Only NMC-approved programmes lead to registration as a nurse in the UK. Non-approved courses do not lead to NMC registration, regardless of the course name or marketing.",
    checks: [
    ...
    nextAction:
      "Confirm the course is on the NMC approved-programmes list before committing time or money.",
  },
  fitCopyRecommended: () =>
    "This route appears structurally relevant to your situation. It is not a promise of NMC registration — confirm entry requirements with the specific NMC-approved provider or employer.",
  fitCopyBackup: () =>
    "A second structurally relevant route. Compare against the recommended route and confirm entry requirements with the provider or employer.",
  ...
  mayOpenLaterFit:
    "A UK training route that may become relevant after the verification step above — not currently a confirmed route for you.",
};
